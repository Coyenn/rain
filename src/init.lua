...
local function connectLoop()
	...
	table.insert(
		...
		RunService.RenderStepped:connect(function()
			...
			local part, position =
				raycast(Ray.new(workspace.CurrentCamera.CFrame.p, -rainDirection * RAIN_SCANHEIGHT), true)
		...
		signal:connect(function()
			...
			if frame >= RAIN_UPDATE_PERIOD then
				...
				local mapped = workspace.CurrentCamera.CFrame:inverse()
					* (workspace.CurrentCamera.CFrame.p - rainDirection)
					...
					if not disabled then
						...
						if not currentCeiling or workspace.CurrentCamera.CFrame.p.y <= currentCeiling then
							...
							for i = 1, #volumeScanGrid do -- In order, so first hit is closest
								if
									not raycast(
										Ray.new(workspace.CurrentCamera.CFrame * volumeScanGrid[i], rayDirection),
										true
									)
								then
			...
			for i = 1, numSplashes do
				...
				local part, position, normal =
					raycast(Ray.new(transform * v3(x, RAIN_OCCLUDECHECK_OFFSET_Y, z), rayDirection))
				...
				elseif inside then
					...
					local corrected = transform
						* v3(
							x,
							rand:NextNumber(RAIN_NOSPLASH_STRAIGHT_OFFSET_Y_MIN, RAIN_NOSPLASH_STRAIGHT_OFFSET_Y_MAX),
							z
						)
					if currentCeiling and corrected.Y > currentCeiling and rainDirection.Y < 0 then
						corrected = corrected + rainDirection * (currentCeiling - corrected.Y) / rainDirection.Y
...
function Rain.new()
	return setmetatable({}, { __index = Rain })
end
